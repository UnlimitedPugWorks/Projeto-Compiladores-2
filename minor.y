...
%%
file	: PROGRAM decls START { func="main"; ret=tPUB + tINT + tFUNC; IDpush(); } main END{ IDpop(); evaluate(binNode(PROGRAM, $2, binNode(FUNCTION, binNode(END, TID(func), TINT(ret)), binNode(FARGS, nilNode(NIL), $5)))); }
...
decl	: qualif const vardecl { $$ = uniNode(VAR, $3); $$->info = $1+$2+$3->info; isCte($$); }
		| FUNCTION qualif ftype ID { isFunc(func = $4, ret = $2+$3+tFUNC); IDpush(); } fvars { IDchange(ret, $4, $6, 1); } eqbody{ $$ = binNode(FUNCTION, binNode(END, TID($4), TINT(ret)), binNode(FARGS, $6, $8)); IDpop(); isFwd(func, ret, $8); }
		| FUNCTION qualif ftype ID { isFunc( func = $4, ret = $2+$3+tFUNC); IDpush(); } eqbody{ $$ = binNode(FUNCTION, binNode(END, TID($4), TINT(ret)), binNode(FARGS, nilNode(NIL), $6)); IDpop(); isFwd(func, ret, $6); }
		| error	{ $$ = nilNode(NIL); }
		;
...
fvars	: fvar			    { $$ = binNode(ARGS, nilNode(NIL), $1);IDnew($1->info, $1->SUB(0)->value.s, $1->SUB(1)); }
		| fvars ';' fvar	{ $$ = binNode(ARGS, $1, $3);IDnew($3->info, $3->SUB(0)->value.s, $3->SUB(1)); }
		;
...
instr	: IF expr THEN block elifs else FI 	{ $$ = binNode(FI, binNode(THEN, binNode(IF, $2, $4), $5), $6); }
		| FOR expr UNTIL expr STEP expr DO { cycle++; } block DONE{ $$ = binNode(FOR, binNode(UNTIL, $2, $4), binNode(STEP, $9, $6)); cycle--; }
